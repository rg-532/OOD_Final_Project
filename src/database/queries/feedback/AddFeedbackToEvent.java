...
public class AddFeedbackToEvent implements ManipulationStrategy {
	
	private boolean result;
	
	//params:
	private Feedback add;
	
	public AddFeedbackToEvent(Feedback add) {
		this.add = add;
	}
	...
	public void editStatement(PreparedStatement pstmt) throws SQLException {
		SelectionContext<Feedback> cnt = new SelectionContext<>(new GetFeedbackForEvent(add.getEventId()));
		int id = cnt.doOperation().size() + 1;
		pstmt.setInt(1, add.getEventId());
		pstmt.setInt(2, id);
		pstmt.setString(3, add.getText());
